...
/** SPI serial device. */
class AP_SerialBus_SPI_MS5803 : public AP_SerialBus
{
public:
    AP_SerialBus_SPI_MS5803(enum AP_HAL::SPIDevice device, enum AP_HAL::SPIDeviceDriver::bus_speed speed);
    void init();
...
/** I2C serial device. */
class AP_SerialBus_I2C_MS5803 : public AP_SerialBus
{
public:
    AP_SerialBus_I2C_MS5803(AP_HAL::I2CDriver *i2c, uint8_t addr);
    void init();
    uint16_t read_16bits(uint8_t reg);
    uint32_t read_24bits(uint8_t reg);
    void write(uint8_t reg);
...
class AP_Baro_MS58XX : public AP_Baro_Backend
{
public:
...
class AP_Baro_MS5803 : public AP_Baro_MS58XX
{
public:
    AP_Baro_MS5803(AP_Baro &baro, AP_SerialBus *serial, bool use_timer);
private:
    void _calculate();
};
