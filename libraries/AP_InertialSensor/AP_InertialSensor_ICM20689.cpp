...
// ICM20689 accelerometer scaling
#define ICM20689_ACCEL_SCALE_1G    (GRAVITY_MSS / 4096.0f)

#if CONFIG_HAL_BOARD == HAL_BOARD_APM2
#define ICM20689_DRDY_PIN 70
...
#define ICM20689_DRDY_PIN BBB_P8_14
#endif
#endif
...
#define ICM20689ES_REV_C4                        0x14    // 0001			0100
#define ICM20689ES_REV_C5                        0x15    // 0001			0101
#define ICM20689ES_REV_D6                        0x16    // 0001			0110
#define ICM20689ES_REV_D7                        0x17    // 0001			0111
#define ICM20689ES_REV_D8                        0x18    // 0001			1000
#define ICM20689_REV_C4                          0x54    // 0101			0100
#define ICM20689_REV_C5                          0x55    // 0101			0101
#define ICM20689_REV_D6                          0x56    // 0101			0110
#define ICM20689_REV_D7                          0x57    // 0101			0111
#define ICM20689_REV_D8                          0x58    // 0101			1000
#define ICM20689_REV_D9                          0x59    // 0101			1001
...
AP_ICM20689_BusDriver_SPI::AP_ICM20689_BusDriver_SPI(void) :
    _error_count(0)
{
    _spi = hal.spi->device(AP_HAL::SPIDevice_ICM20689);
}

void AP_ICM20689_BusDriver_SPI::init(bool &fifo_mode, uint8_t &max_samples)
{
    fifo_mode = false;
    _error_count = 0;
...
void AP_ICM20689_BusDriver_SPI::read8(uint8_t reg, uint8_t *val)
{
...
void AP_ICM20689_BusDriver_SPI::write8(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
...
void AP_ICM20689_BusDriver_SPI::set_bus_speed(AP_HAL::SPIDeviceDriver::bus_speed speed)
{
    _spi->set_bus_speed(speed);
}

void AP_ICM20689_BusDriver_SPI::read_burst(uint8_t *samples,
...
AP_HAL::Semaphore* AP_ICM20689_BusDriver_SPI::get_semaphore()
{
    return _spi->get_semaphore();
}

/* I2C bus driver implementation */
AP_ICM20689_BusDriver_I2C::AP_ICM20689_BusDriver_I2C(AP_HAL::I2CDriver *i2c, uint8_t addr) :
    _addr(addr),
    _i2c(i2c),
    _i2c_sem(NULL)
{}

void AP_ICM20689_BusDriver_I2C::init(bool &fifo_mode, uint8_t &max_samples)
{
    // enable fifo mode
    fifo_mode = true;
     ...
     * accel_z
     */
    max_samples = ICM20689_MAX_FIFO_SAMPLES;
}

void AP_ICM20689_BusDriver_I2C::read8(uint8_t reg, uint8_t *val)
{
    _i2c->readRegister(_addr, reg, val);
}

void AP_ICM20689_BusDriver_I2C::write8(uint8_t reg, uint8_t val)
{
    _i2c->writeRegister(_addr, reg, val);
}

void AP_ICM20689_BusDriver_I2C::set_bus_speed(AP_HAL::SPIDeviceDriver::bus_speed speed)
{}

void AP_ICM20689_BusDriver_I2C::read_burst(uint8_t *samples,
                                          AP_HAL::DigitalSource *_drdy_pin,
                                          uint8_t &n_samples)
{
	uint16_t bytes_read;
    ...
    if(ret != 0) {
        hal.console->printf_P(PSTR("ICM20689: error in i2c read\n"));
        n_samples = 0;
        return;
    }
    ...
    n_samples = bytes_read / ICM20689_SAMPLE_SIZE;
    ...
    if(n_samples > 3) {
        ...
        ret = _i2c->readRegisters(_addr, MPUREG_FIFO_R_W, n_samples * ICM20689_SAMPLE_SIZE, _rx);
    }

    if(ret != 0) {
        hal.console->printf_P(PSTR("ICM20689: error in i2c read %d bytes\n"),
                                   n_samples * ICM20689_SAMPLE_SIZE);
        n_samples = 0;
        return;
    }

    memcpy(samples, _rx, n_samples * ICM20689_SAMPLE_SIZE);

    return;
}

AP_HAL::Semaphore* AP_ICM20689_BusDriver_I2C::get_semaphore()
{
 ...
 */
const float AP_InertialSensor_ICM20689::_gyro_scale = (0.0174532f / 16.4f);
...
AP_InertialSensor_ICM20689::AP_InertialSensor_ICM20689(AP_InertialSensor &imu, AP_ICM20689_BusDriver *bus) :
...
AP_InertialSensor_ICM20689::~AP_InertialSensor_ICM20689()
{
    delete _bus;
}
 ...
 * SPIDriver table */
AP_InertialSensor_Backend *AP_InertialSensor_ICM20689::detect_spi(AP_InertialSensor &imu)
{
    AP_ICM20689_BusDriver *bus = new AP_ICM20689_BusDriver_SPI();
    if (!bus)
        return nullptr;
...
AP_InertialSensor_Backend *AP_InertialSensor_ICM20689::detect_i2c(AP_InertialSensor &imu,
                                                                 AP_HAL::I2CDriver *i2c,
                                                                 uint8_t addr)
{
    AP_ICM20689_BusDriver *bus = new AP_ICM20689_BusDriver_I2C(i2c, addr);
    if (!bus)
        return nullptr;
    return _detect(imu, bus);
}
...
AP_InertialSensor_Backend *AP_InertialSensor_ICM20689::_detect(AP_InertialSensor &_imu,
                                                              AP_ICM20689_BusDriver *bus)
{
    AP_InertialSensor_ICM20689 *sensor = new AP_InertialSensor_ICM20689(_imu, bus);
    if (sensor == NULL) {
        delete bus;
        return nullptr;
    }
...
bool AP_InertialSensor_ICM20689::_init_sensor(void)
{
...
#ifdef ICM20689_DRDY_PIN
    _drdy_pin = hal.gpio->channel(ICM20689_DRDY_PIN);
    ...
    do {
        ...
        if (tries++ > 5) {
            hal.console->print_P(PSTR("failed to boot ICM20689 5 times")); 
            return false;
        }
    } while (1);
...
{    
    ...
    accel *= ICM20689_ACCEL_SCALE_1G / num_samples;
...
/**
 * Return true if the ICM20689 has new data available for reading.
 *
 * We use the data ready pin if it is available.  Otherwise, read the
 * status register.
 */
bool AP_InertialSensor_ICM20689::_data_ready()
{
    if (_drdy_pin) {
...
void AP_InertialSensor_ICM20689::_accumulate(uint8_t *samples, uint8_t n_samples)
{
    for(uint8_t i=0; i < n_samples; i++) {
        uint8_t *data = samples + ICM20689_SAMPLE_SIZE * i;
#if ICM20689_FAST_SAMPLING
...
uint8_t AP_InertialSensor_ICM20689::_register_read( uint8_t reg )
{
    uint8_t val;
...
void AP_InertialSensor_ICM20689::_register_write(uint8_t reg, uint8_t val)
{
    _bus->write8(reg, val);
}
 ...
 */
void AP_InertialSensor_ICM20689::_register_write_check(uint8_t reg, uint8_t val)
{
    uint8_t readed;
 ...
 */
void AP_InertialSensor_ICM20689::_set_filter_register(uint16_t filter_hz)
{
    uint8_t filter;
...
bool AP_InertialSensor_ICM20689::_hardware_init(void)
{
    ...
    if (!_bus_sem->take(100)) {
        hal.scheduler->panic(PSTR("ICM20689: Unable to get semaphore"));
    }
    ...
    _samples = new uint8_t[max_samples * ICM20689_SAMPLE_SIZE];
    hal.scheduler->delay(1);
    ...
    if ((_product_id == ICM20689ES_REV_C4) || 
        (_product_id == ICM20689ES_REV_C5) ||
        (_product_id == ICM20689_REV_C4)   || 
        (_product_id == ICM20689_REV_C5)) {
...
void AP_InertialSensor_ICM20689::_dump_registers(void)
{
