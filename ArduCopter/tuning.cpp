...
//  AB ZhaoYJ@2016-11-03 for using rc_8 to tuning instead of rc_6(ch6)
#define TUNING_CH8
void Copter::tuning() {
    ...
    g.rc_8.set_range(g.radio_tuning_low,g.radio_tuning_high);
    float tuning_value = (float)g.rc_8.control_in / 1000.0f;
    ...
    Log_Write_Parameter_Tuning(g.radio_tuning, tuning_value, g.rc_8.control_in, g.radio_tuning_low, g.radio_tuning_high);
#else
    // set tuning range and then get new value
...
#ifdef TUNING_CH8
        wp_nav.set_speed_xy(g.rc_8.control_in);
#else
...
#ifdef TUNING_CH8
        motors.ext_gyro_gain(g.rc_8.control_in);
#else
...
#ifdef TUNING_CH8
        compass.set_declination(ToRad((2.0f * g.rc_8.control_in - g.radio_tuning_high)/100.0f), false);     // 2nd parameter is false because we do not want to save to eeprom because this would have a performance impact
#else
...
#ifdef TUNING_CH8
        circle_nav.set_rate((float)g.rc_8.control_in/25.0f-20.0f);
#else
...
#ifdef TUNING_CH8
        g.rc_feel_rp = g.rc_8.control_in / 10;
#else
