...
bool Copter::guided_init(bool ignore_checks)
{
    ...
    if (position_ok() || ignore_checks) {
        ...
        // AB ZhaoYJ@2017-04-05 for copying loiter
...
#define TEST_PRT_GUIDED_Z 0
#if TEST_PRT_GUIDED_Z
    static uint32_t cnt_gz = 0;
    bool prt_en = (0 == (cnt_gz++%200));
#endif
#endif
    if (!failsafe.radio) {
        // get pilot's desired yaw rate
        ...
        if(prt_en)
        {
            printf("[%d ms] guided: cr: %f\n", hal.scheduler->millis(), target_climb_rate);
        }
#endif
        ...
        if(prt_en)
        {
            printf("[%d ms] guided(constrain): cr: %f\n", hal.scheduler->millis(), target_climb_rate);
        }
#endif
        //
#endif
    ...
    if(prt_en)
    {
        printf("[%d ms] semi-auto: pos_c: vel_desired: %f\n", hal.scheduler->millis(), pos_control.get_desired_velocity().z);
    }
#endif
#endif
    pos_control.update_z_controller();
