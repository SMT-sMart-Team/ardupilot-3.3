...
#define NON_GUIDED 0xFF
    ...
    static uint8_t unconfirm_mode = NON_GUIDED;
    static uint32_t confirm_mode_enter_start;
    static uint32_t confirm_mode_quit_start;

    static uint8_t guided_status = IN_NONE;
    static uint8_t last_guided_status = IN_IDLE;
    static uint8_t unconfirm_status = IN_IDLE;
    static uint32_t confirm_tms_start;

    static bool enter_guided = false;

    if(g.rc_8.radio_in > (g.rc_8.radio_trim + 300))
    {
        if(GUIDED == unconfirm_mode)
        {

            if(TIME32_SUB(hal.scheduler->millis(), confirm_mode_enter_start ) > CONFIRM_TMS)
            {
                enter_guided = true;
            }
        }
        ...
        {
            unconfirm_mode = GUIDED;
            confirm_mode_enter_start = hal.scheduler->millis();
        }
    }
    else
    {
        if(NON_GUIDED == unconfirm_mode)
        {

            if(TIME32_SUB(hal.scheduler->millis(), confirm_mode_quit_start ) > CONFIRM_TMS)
            {
                enter_guided = false;
            }
        }
        ...
        {
            unconfirm_mode = NON_GUIDED; 
            confirm_mode_quit_start = hal.scheduler->millis();
        }
    }
    ...
    if(enter_guided)
    {
        ...
        {
            // if forward
            // if(channel_pitch->radio_in > 1700)
            if(channel_pitch->radio_in > (channel_pitch->radio_trim + VWP_RADIO_DZ) )
            {
                if(IN_FORWARD == unconfirm_status)
                {
                    // debounce OK
                    if(TIME32_SUB(hal.scheduler->millis(), confirm_tms_start ) > CONFIRM_TMS)
                    {
                        guided_status = IN_FORWARD;
                    }
                }
                else // radio in change
                {
                    confirm_tms_start = hal.scheduler->millis();
                    unconfirm_status = IN_FORWARD;
                }
            ...
            // if backward
            // else if(channel_pitch->radio_in < 1300)
            else if(channel_pitch->radio_in < (channel_pitch->radio_trim - VWP_RADIO_DZ) )
            {
                if(IN_BACKWARD == unconfirm_status)
                {
                    // debounce OK
                    if(TIME32_SUB(hal.scheduler->millis(), confirm_tms_start ) > CONFIRM_TMS)
                    {
            ...
            {
                if(IN_IDLE == unconfirm_status)
                {
                    // debounce OK
                    if(TIME32_SUB(hal.scheduler->millis(), confirm_tms_start ) > CONFIRM_TMS)
                    {
                        guided_status = IN_IDLE;
                    }
                }
                else // radio in change
                {
